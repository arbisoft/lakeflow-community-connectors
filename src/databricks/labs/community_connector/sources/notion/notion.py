...
class NotionLakeflowConnect(LakeflowConnect):
    ...
    def list_tables(self) -> List[str]:
        ...
        return ["pages", "databases", "blocks", "users", "comments"]
    ...
    def _read_incremental(
        ...
        while len(records) < max_records:
            if table_name in ("pages", "databases"):
                # Use search endpoint with POST
                body = {
                    "filter": {"property": "object", "value": table_name[:-1] if table_name != "databases" else "database"},
                ...
                # Blocks require a parent block_id - this is a simplified version
                # In practice, blocks are fetched per-page
                if not start_cursor:
