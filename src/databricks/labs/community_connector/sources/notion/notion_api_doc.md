...
| `databases` | Database definitions and metadata | Yes |
| `blocks` | Block content within pages | Yes |
| `users` | Workspace users | No (snapshot) |
| `comments` | Page/block comments | Yes |
| `data_sources` | External data source connections | Yes |

**Object List Retrieval**: Objects are discovered via the `/search` endpoint which returns pages and databases.
...
Databases define structured data with property templates.
...
| `databases` | `cdc` | `last_edited_time` |
...
### Search Endpoint (Pages & Databases)

**Endpoint**: `POST /search`

**Purpose**: Discover pages and databases accessible to the integration.
...
**Purpose**: Query database contents with filters and sorts.
...
**Cursor Field**: `last_edited_time` for pages/databases/blocks, `created_time` for comments.
...
3. **Permission Errors**: 404 with "Make sure the relevant pages and databases are shared with your integration" should be ignored (not a connector error).
